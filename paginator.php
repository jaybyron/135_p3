...
class Paginator
{
    ...
    private function _getPageNav()
    {
        ...
        return '<div class="pagination"><ul class="nav nav-pills">' . $this->_previousLink($start) . $this->_page($start, $end) . $this->_nextLink($end) . '</ul></div>';
    }
    ...
    private function _page($start, $end)
    {
        ...
        for ($i = $start; $i <= $end; $i++) {
            $result .= '<li ' . (($i == $this->_currentPage) ? 'class="enabled"' : "") . '><a href="' . $this->_link . '?current=' . $i . '">' . $i . '</a></li>';
        }
        return $result;
    }
